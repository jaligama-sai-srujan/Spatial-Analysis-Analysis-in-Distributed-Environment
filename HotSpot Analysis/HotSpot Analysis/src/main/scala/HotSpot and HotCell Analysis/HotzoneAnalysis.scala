...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    pointDf = spark.sql("select trim(_c5) as _c5 from point").persist();
    pointDf.createOrReplaceTempView("point")
    ...
    val joinDf = spark.sql("select rectangle._c0 as rectangle, point._c5 as point from rectangle,point where ST_Contains(rectangle._c0,point._c5)").persist();
    joinDf.createOrReplaceTempView("joinResult")

    // YOU NEED TO CHANGE THIS PART
	val resultDf = spark.sql("select rectangle,count(*) as totalpoints from joinResult group by rectangle order by rectangle").persist();
	resultDf.createOrReplaceTempView("HotzoneResult")
	resultDf.show()
